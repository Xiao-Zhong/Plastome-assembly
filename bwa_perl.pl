...
if ($inpe){
	...
	for (my $i=0; $i<@inpe; $i +=2){
		...
		print OUT "bwa mem $inref $inpe[$i] $inpe[$i+1] > $outdir/$sample.pe$i.sam; samtools view -S -b -o $outdir/$sample.pe$i.bam $outdir/$sample.pe$i.sam; samtools sort -o $outdir/$sample.pe$i.sort.bam $outdir/$sample.pe$i.bam; samtools index $outdir/$sample.pe$i.sort.bam\n";
...
if ($inse){
	my @inse=split(/,/, $inse);
	for (my $i=0; $i<@inse; $i++){
		print OUT "bwa mem $inref $inse[$i] > $outdir/$sample.se$i.sam; samtools view -S -b -o $outdir/$sample.se$i.bam $outdir/$sample.se$i.sam; samtools sort -o $outdir/$sample.se$i.sort.bam $outdir/$sample.se$i.bam; samtools index $outdir/$sample.se$i.sort.bam\n";
