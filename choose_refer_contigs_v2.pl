...
## add the process "filter overlaps in alignment to accurately calculate coverage".
...
foreach my $rcid(keys %align_stat){
	# ID: reference_id	contigs_set_id
	# Factors: reference_covered	reference_length	contigs_covered	contigs_number	total_length_of_contigs	contigs_length(final column)
	# Scores: reference_covered	reference_length	contigs_covered	contigs_number	total_length_of_contigs	contigs_length	bonus	total_score
	print "$rcid\t";
	...
	# No. of contigs
	my $cn_score=0;
	$cn_score +=100 if($align_stat{$rcid}[3] ==3 && $align_stat{$rcid}[5]>=100000); 
	$cn_score +=50 if($align_stat{$rcid}[3] <=4 && $align_stat{$rcid}[3] !=3 && $align_stat{$rcid}[5]>=70000);
	$cn_score +=20 if($align_stat{$rcid}[3] ==5 && $align_stat{$rcid}[5]>=80000);
	$cn_score -=20 if($align_stat{$rcid}[3] >=10);
	print "$cn_score\t";

	# total length of contigs
	my $ctl_score=0;
	#$ctl_score +=40 if ($align_stat{$rcid}[5]>110000);
	$ctl_score +=30 if ($align_stat{$rcid}[5]>100000 && $align_stat{$rcid}[5]<200000 );
	$ctl_score -=50 if ($align_stat{$rcid}[5]<=30000 || $align_stat{$rcid}[5]>=200000);
	print "$ctl_score\t";
