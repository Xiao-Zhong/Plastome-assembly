...
use lib "/ifs4/BC_PUB/biosoft/pipe/bc_ba/Annotation/bin/Annotation_pipeline1_1.0/01.gene_finding/protein-map-genome/bin/../lib";
use Data::Dumper;
...
$Bin="/ifs4/BC_PUB/biosoft/pipe/bc_ba/Annotation/bin/Annotation_pipeline1_1.0/01.gene_finding/protein-map-genome/bin";
...
if ($Step =~/1/){
	...
	if ($Run eq "qsub") {
		`perl $qsub_sge $QP_para --reqsub --maxjob $Cpu $Node_para  $tblastn_shell_file`;
	}
	if ($Run eq "multi") {
...
if ($Step =~/2/){
	...
	#`perl $Bin/genomic_cluster.pl  -overlap_percent 0.9 $Outdir/$Qr_file_basename.blast.solar.filter.table > $Outdir/$Qr_file_basename.blast.solar.filter.table.nonredundance`;
	...
	`ln -s $Outdir/$Qr_file_basename.blast.solar.filter $Outdir/$Qr_file_basename.blast.solar.filter.nr`;	
