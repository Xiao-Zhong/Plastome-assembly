	...
	if(!exists $cidhash{$cid}){
		...
		if(@order>=2){
			...
			if($i<@order-1){
				...
				if ($s ne $next_s){
					my $gap_in_ref=$listhash{$next_cid}{$next_s}[0][0]-$listhash{$cid}{$s}[-1][1]-1;
					if ($gap_in_ref < 0){
						if ($s eq "+"){
							$start=1;
						...
						my $extend_len;
						if ($s eq "+"){
							$extend_len=$listhash{$cid}{$s}[-1][12]-$listhash{$cid}{$s}[-1][4];
						}else{
							$extend_len=$listhash{$cid}{$s}[-1][4]-1;
						}

						if($extend_len > $gap_in_ref + 2000 ){
							if ($s eq "+"){
								$start=1;
								$end=$listhash{$cid}{$s}[-1][4];
							}else{
